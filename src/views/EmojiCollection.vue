...
const setWaterfallData = async () => {
  ...
  if (W !== Width || init) {
    init = false;
    waterfallData.column =
      Math.floor(waterfallBox.value!.clientWidth / Width) >= 1
        ? Math.floor(waterfallBox.value!.clientWidth / Width)
        : 1;
...
const changeWaterfallList = async () => {
  ...
  waterfallData.waterfallList = Array.from(Array(column), () => new Array(0));
  waterfallData.initData.forEach(item => {
    waterfallData.waterfallIndex = waterfallData.waterfallHeightList.indexOf(
      Math.min(...waterfallData.waterfallHeightList),
    );
    waterfallData.waterfallList[waterfallData.waterfallIndex].push(item);
    waterfallData.waterfallHeightList[waterfallData.waterfallIndex] +=
      item.height;
  });
  await nextTick();
  lazyLoad();
...
const lazyLoad = () => {
  ...
  for (let i = 0; i < length; i++) {
    waterfallBox
      .value!.children[i].querySelectorAll('.waterfall-item-img')
      .forEach(ele => {
